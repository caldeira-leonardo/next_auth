import { generateId } from './utils';

export const fieldTypes = {
  title: {
    input_type: 'input_text',
    label: 'Título',
    field_name: 'title',
    options: {
      props: [['required', true]],
    },
  },
  name: {
    input_type: 'input_text',
    label: 'Nome',
    field_name: 'name',
    options: {
      props: [['required', true]],
    },
  },
  description: {
    input_type: 'input_text',
    label: 'Descrição',
    field_name: 'description',
    options: {
      type: 'textarea',
      props: [
        ['maxLength', 140],
        ['rows', 5],
      ],
      class: 'full-content-adapt',
    },
  },
  email: {
    input_type: 'input_text',
    label: 'Email',
    field_name: 'email',
    options: {
      props: [['type', 'email']],
    },
  },

  phone: {
    input_type: 'input_text',
    label: 'Telefone',
    field_name: 'phone',
    options: {
      masks: ['phone'],
      props: [],
    },
  },
  cpf: {
    input_type: 'input_text',
    label: 'CPF',
    field_name: 'cpf',
    options: {
      masks: ['cpf'],
      props: [
        ['required', true],
        ['type', 'cpf'],
      ],
    },
  },
  cnpj: {
    input_type: 'input_text',
    label: 'CNPJ',
    field_name: 'cnpj',
    options: {
      masks: ['cnpj'],
      props: [
        ['required', true],
        ['type', 'cnpj'],
      ],
    },
  },
  cpfCnpj: {
    input_type: 'input_text',
    label: 'CPF/CNPJ',
    field_name: 'document',
    options: {
      masks: ['cpf', 'cnpj'],
      props: [
        ['required', true],
        ['type', 'cpfCnpj'],
      ],
    },
  },
  currency: {
    input_type: 'input_text',
    label: 'Valor',
    field_name: 'amount',
    options: {
      rtl: true,
      props: [],
    },
  },
  date: {
    input_type: 'input_text',
    label: 'Data',
    field_name: 'date',
    options: {
      props: [['type', 'date']],
    },
  },
  futureDate: {
    input_type: 'input_text',
    label: 'Data',
    field_name: 'date',
    options: {
      props: [
        ['type', 'date'],
        ['min', new Date().toISOString().split('T')[0]],
      ],
    },
  },
  fileUpload: {
    input_type: 'input_image',
    label: 'Anexos',
    field_name: 'attachments',
    options: {
      accept: 'image/*,video/*,.pdf,.doc,.docx',
      props: [['multiple', true]],
    },
  },
  removeButton: {
    input_type: 'button',
    label: '',
    options: {
      class:
        'btn-danger rounded-circle round-20 d-inline-flex align-items-center justify-content-center px-2 removeLIneBtn',
      props: [
        ['type', 'button'],
        ['title', 'Remover item'],
      ],
    },
  },

  boletoType: {
    input_type: 'input_select',
    label: 'Tipo do Boleto',
    field_name: 'type',
    items: [
      { key: '', label: 'Selecione o tipo do boleto', disabled: true },
      { key: 'common', label: 'Comum' },
      { key: 'hybrid', label: 'Híbrido (com QRCode)' },
    ],
    options: {
      props: [['required', true]],
      defaultValue: 'common',
    },
  },

  dueDate: {
    input_type: 'input_text',
    label: 'Data de Vencimento',
    field_name: 'due_date',
    options: {
      props: [
        ['required', true],
        ['type', 'date'],
        ['min', new Date().toISOString().split('T')[0]],
      ],
    },
  },

  myNumber: {
    input_type: 'input_text',
    label: 'Meu Número',
    field_name: 'my_number',
    options: {
      props: [['placeholder', 'Opcional']],
    },
  },

  state: {
    input_type: 'input_select',
    label: 'Estado (UF)',
    field_name: 'payer_state',
    items: [
      { key: '', label: 'Selecione', disabled: true, selected: true },
      { key: 'AC', label: 'AC' },
      { key: 'AL', label: 'AL' },
      { key: 'AP', label: 'AP' },
      { key: 'AM', label: 'AM' },
      { key: 'BA', label: 'BA' },
      { key: 'CE', label: 'CE' },
      { key: 'DF', label: 'DF' },
      { key: 'ES', label: 'ES' },
      { key: 'GO', label: 'GO' },
      { key: 'MA', label: 'MA' },
      { key: 'MT', label: 'MT' },
      { key: 'MS', label: 'MS' },
      { key: 'MG', label: 'MG' },
      { key: 'PA', label: 'PA' },
      { key: 'PB', label: 'PB' },
      { key: 'PR', label: 'PR' },
      { key: 'PE', label: 'PE' },
      { key: 'PI', label: 'PI' },
      { key: 'RJ', label: 'RJ' },
      { key: 'RN', label: 'RN' },
      { key: 'RS', label: 'RS' },
      { key: 'RO', label: 'RO' },
      { key: 'RR', label: 'RR' },
      { key: 'SC', label: 'SC' },
      { key: 'SP', label: 'SP' },
      { key: 'SE', label: 'SE' },
      { key: 'TO', label: 'TO' },
    ],
    options: {
      props: [['required', true]],
    },
  },

  complement: {
    input_type: 'input_text',
    label: 'Complemento',
    field_name: 'payer_complement',
    options: {
      props: [['placeholder', 'Complemento']],
    },
  },

  neighborhood: {
    input_type: 'input_text',
    label: 'Bairro',
    field_name: 'payer_neighborhood',
    options: {
      props: [
        ['required', true],
        ['placeholder', 'Nome do bairro'],
      ],
    },
  },

  city: {
    input_type: 'input_text',
    label: 'Cidade',
    field_name: 'payer_city',
    options: {
      props: [
        ['required', true],
        ['placeholder', 'Nome da cidade'],
      ],
    },
  },

  zipcode: {
    input_type: 'input_text',
    label: 'CEP',
    field_name: 'payer_zipcode',
    options: {
      masks: ['cep'],
      props: [['placeholder', '00000-000']],
      onChange: async (value, updateField) => {
        const cleanCep = value.replace(/[^\d]/g, '');
        if (cleanCep.length === 8) {
          try {
            const response = await fetch(`https://viacep.com.br/ws/${cleanCep}/json/`);
            const data = await response.json();

            if (!data.erro) {
              updateField('payer_street', data.logradouro || '');
              updateField('payer_neighborhood', data.bairro || '');
              updateField('payer_city', data.localidade || '');
              updateField('payer_state', data.uf || '');
            }
          } catch (error) {
            console.error('Erro ao buscar CEP:', error);
          }
        }
      },
    },
  },
};

export const receipts = {
  favorite_payer: [
    {
      input_type: 'container',
      label: '',
      options: {
        class: 'row mb-3',
      },
      items: [
        {
          ...fieldTypes.name,
          label: 'Nome Completo',
          field_name: 'payer_name',
          options: {
            ...fieldTypes.name.options,
            props: [...fieldTypes.name.options.props, ['placeholder', 'Nome do pagador']],
            class: 'col-md-8',
          },
        },
        {
          ...fieldTypes.cpfCnpj,
          field_name: 'payer_document',
          options: {
            ...fieldTypes.cpfCnpj.options,
            props: [...fieldTypes.cpfCnpj.options.props, ['placeholder', 'CPF/CNPJ do pagador']],
            class: 'col-md-4',
          },
        },
      ],
    },
    {
      input_type: 'container',
      label: '',
      options: {
        class: 'row mb-3',
      },
      items: [
        {
          ...fieldTypes.email,
          field_name: 'payer_email',
          options: {
            ...fieldTypes.email.options,
            props: [...fieldTypes.email.options.props, ['placeholder', 'Email do pagador']],
            class: 'col-md-8',
          },
        },
        {
          ...fieldTypes.phone,
          field_name: 'payer_phone',
          options: {
            ...fieldTypes.phone.options,
            props: [['placeholder', '(00) 00000-0000']],
            class: 'col-md-4',
          },
        },
      ],
    },
    {
      input_type: 'container',
      label: '',
      options: {
        class: 'row mb-3',
      },
      items: [
        {
          ...fieldTypes.zipcode,
          options: { ...fieldTypes.zipcode.options, class: 'col-md-3' },
        },
        {
          input_type: 'input_text',
          label: 'Rua',
          field_name: 'payer_street',
          options: {
            props: [['placeholder', 'Nome da rua']],
            class: 'col-md-5',
          },
        },
        {
          input_type: 'input_text',
          label: 'Número',
          field_name: 'payer_number',
          options: {
            props: [['placeholder', '0000']],
            class: 'col-md-2',
          },
        },
        {
          input_type: 'input_text',
          label: 'Complemento',
          field_name: 'payer_complement',
          options: {
            props: [['placeholder', '0000']],
            class: 'col-md-2',
          },
        },
      ],
    },
    {
      input_type: 'container',
      label: '',
      options: {
        class: 'row mb-3',
      },
      items: [
        {
          input_type: 'input_text',
          label: 'Bairro',
          field_name: 'payer_neighborhood',
          options: {
            props: [['placeholder', 'Bairro']],
            class: 'col-md-4',
          },
        },
        {
          input_type: 'input_text',
          label: 'Cidade',
          field_name: 'payer_city',
          options: {
            props: [['placeholder', 'Cidade']],
            class: 'col-md-4',
          },
        },
        {
          input_type: 'input_text',
          label: 'Estado (UF)',
          field_name: 'payer_state',
          options: {
            props: [['placeholder', 'Estado (UF)']],
            class: 'col-md-4',
          },
        },
      ],
    },
  ],

  single_boleto: [
    {
      input_type: 'container',
      label: 'Informações Básicas',
      options: { class: 'row mb-4' },
      items: [
        {
          ...fieldTypes.boletoType,
          options: { ...fieldTypes.boletoType.options, class: 'col-md-6' },
        },
        {
          ...fieldTypes.currency,
          label: 'Valor do Boleto',
          field_name: 'amount',
          options: { ...fieldTypes.currency.options, props: [['required', true]], class: 'col-md-6' },
        },
      ],
    },
    {
      input_type: 'container',
      label: '',
      options: { class: 'row mb-4' },
      items: [
        {
          ...fieldTypes.dueDate,
          options: { ...fieldTypes.dueDate.options, class: 'col-md-6' },
        },
        {
          ...fieldTypes.myNumber,
          options: { ...fieldTypes.myNumber.options, class: 'col-md-6' },
        },
      ],
    },
    {
      input_type: 'container',
      label: 'Dados do Pagador',
      options: { class: 'mb-4' },
      items: [
        {
          input_type: 'input_autocomplete',
          label: 'Pagador Favorito',
          field_name: 'favorite_payer',
          options: {
            items: [
              {
                id: '1',
                name: 'João Silva',
                payer_name: 'João Silva',
                payer_document: '123.456.789-00',
                payer_phone: '(11) 99999-9999',
                payer_email: 'joao@email.com',
                payer_street: 'Rua das Flores',
                payer_number: '123',
                payer_complement: 'Apto 45',
                payer_neighborhood: 'Centro',
                payer_city: 'São Paulo',
                payer_state: 'SP',
                payer_zipcode: '01234-567',
              },
              {
                id: '2',
                name: 'Maria Santos',
                payer_name: 'Maria Santos',
                payer_document: '987.654.321-00',
                payer_phone: '(11) 88888-8888',
                payer_email: 'maria@email.com',
                payer_street: 'Av. Paulista',
                payer_number: '1000',
                payer_complement: 'Sala 501',
                payer_neighborhood: 'Bela Vista',
                payer_city: 'São Paulo',
                payer_state: 'SP',
                payer_zipcode: '01310-100',
              },
              {
                id: '3',
                name: 'Pedro Oliveira',
                payer_name: 'Pedro Oliveira',
                payer_document: '111.222.333-44',
                payer_phone: '(11) 77777-7777',
                payer_email: 'pedro@email.com',
                payer_street: 'Rua Augusta',
                payer_number: '456',
                payer_complement: '',
                payer_neighborhood: 'Consolação',
                payer_city: 'São Paulo',
                payer_state: 'SP',
                payer_zipcode: '01305-000',
              },
              {
                id: '4',
                name: 'Ana Costa',
                payer_name: 'Ana Costa',
                payer_document: '555.666.777-88',
                payer_phone: '(11) 66666-6666',
                payer_email: 'ana@email.com',
                payer_street: 'Rua Oscar Freire',
                payer_number: '789',
                payer_complement: 'Loja 12',
                payer_neighborhood: 'Jardins',
                payer_city: 'São Paulo',
                payer_state: 'SP',
                payer_zipcode: '01426-001',
              },
            ],
            placeholder: 'Selecione um pagador favorito...',
            searchKey: 'name',
            valueKey: 'id',
            displayKey: 'name',
            onChange: async (value, updateField, field) => {
              const selectedItem = field.options.items.find((item) => item.id === value);
              if (selectedItem) {
                updateField('payer_id', selectedItem.payer_id || '');
                updateField('payer_name', selectedItem.payer_name || '');
                updateField('payer_document', selectedItem.payer_document || '');
                updateField('payer_phone', selectedItem.payer_phone || '');
                updateField('payer_email', selectedItem.payer_email || '');
                updateField('payer_street', selectedItem.payer_street || '');
                updateField('payer_number', selectedItem.payer_number || '');
                updateField('payer_complement', selectedItem.payer_complement || '');
                updateField('payer_neighborhood', selectedItem.payer_neighborhood || '');
                updateField('payer_city', selectedItem.payer_city || '');
                updateField('payer_state', selectedItem.payer_state || '');
                updateField('payer_zipcode', selectedItem.payer_zipcode || '');
              }
            },
            class: 'col-md-12',
            props: [],
          },
        },
      ],
    },
    {
      input_type: 'container',
      label: '',
      options: { class: 'row mb-4' },
      items: [
        {
          ...fieldTypes.name,
          label: 'Nome do Pagador',
          field_name: 'payer_name',
          options: {
            ...fieldTypes.name.options,
            class: 'col-md-6',
            props: [
              ['required', true],
              ['minLength', 3],
              ['maxLength', 40],
            ],
          },
        },
        {
          ...fieldTypes.cpfCnpj,
          field_name: 'payer_document',
          options: {
            ...fieldTypes.cpfCnpj.options,
            class: 'col-md-6',
            props: [['required', true]],
          },
        },
      ],
    },
    {
      input_type: 'container',
      label: '',
      options: { class: 'row mb-4' },
      items: [
        {
          ...fieldTypes.phone,
          field_name: 'payer_phone',
          options: {
            ...fieldTypes.phone.options,
            class: 'col-md-6',
            props: [
              ['minLength', 9],
              ['maxLength', 11],
            ],
          },
        },
        {
          ...fieldTypes.email,
          label: 'E-mail',
          field_name: 'payer_email',
          options: {
            ...fieldTypes.email.options,
            class: 'col-md-6',
            props: [
              ['required', true],
              ['type', 'email'],
              ['minLength', 3],
              ['maxLength', 100],
            ],
          },
        },
      ],
    },
    {
      input_type: 'container',
      label: 'Endereço',
      options: { class: 'row mb-4' },
      items: [
        {
          ...fieldTypes.zipcode,
          options: {
            ...fieldTypes.zipcode.options,
            props: [...fieldTypes.zipcode.options.props, ['required', true]],
            class: 'col-md-3',
          },
        },
        {
          input_type: 'input_text',
          label: 'Endereço',
          field_name: 'payer_street',
          options: {
            props: [
              ['required', true],
              ['minLength', 3],
              ['maxLength', 40],
              ['placeholder', 'Nome da rua'],
            ],
            class: 'col-md-5',
          },
        },
        {
          input_type: 'input_text',
          label: 'Número',
          field_name: 'payer_number',
          options: {
            props: [
              ['required', true],
              ['minLength', 3],
              ['maxLength', 40],
              ['placeholder', 'Número'],
            ],
            class: 'col-md-2',
          },
        },
        {
          input_type: 'input_text',
          label: 'Complemento',
          field_name: 'payer_complement',
          options: {
            props: [
              ['minLength', 3],
              ['maxLength', 40],
              ['placeholder', 'Complemento'],
            ],
            class: 'col-md-2',
          },
        },
      ],
    },
    {
      input_type: 'container',
      label: '',
      options: { class: 'row mb-4' },
      items: [
        {
          ...fieldTypes.neighborhood,
          options: {
            ...fieldTypes.neighborhood.options,
            class: 'col-md-4',
            props: [
              ['required', true],
              ['minLength', 3],
              ['maxLength', 20],
            ],
          },
        },
        {
          ...fieldTypes.city,
          options: {
            ...fieldTypes.city.options,
            class: 'col-md-4',
            props: [
              ['required', true],
              ['minLength', 3],
              ['maxLength', 20],
            ],
          },
        },
        {
          ...fieldTypes.state,
          options: {
            ...fieldTypes.state.options,
            class: 'col-md-4',
            props: [
              ['required', true],
              ['maxLength', 2],
            ],
          },
        },
      ],
    },
    {
      input_type: 'container',
      label: '',
      options: { class: 'row mb-4' },
      items: [
        {
          input_type: 'input_checkbox',
          label: 'Salvar dados do pagador como favorito',
          field_name: 'save_payer_as_favorite',
          options: {
            helper_text: 'Marque esta opção para salvar os dados do pagador como favorito para uso futuro',
            class: 'col-md-12',
            props: [],
            onChange: async (value, updateField) => {
              console.log('onChange callback called with:', { value, updateField });
            },
          },
        },
      ],
    },
    {
      input_type: 'container',
      label: 'Mensagens do Boleto',
      class: 'mb-4',
      items: [
        {
          input_type: 'input_text',
          label: 'Mensagem 1',
          field_name: 'message1',
          options: {
            class: 'mb-4',
            props: [
              ['placeholder', 'Mensagem 1 (opcional)'],
              ['maxLength', 78],
            ],
          },
        },
        {
          input_type: 'input_text',
          label: 'Mensagem 2',
          field_name: 'message2',
          options: {
            class: 'mb-4',
            props: [
              ['placeholder', 'Mensagem 2 (opcional)'],
              ['maxLength', 78],
            ],
          },
        },
        {
          input_type: 'input_text',
          label: 'Mensagem 3',
          field_name: 'message3',
          options: {
            class: 'mb-4',
            props: [
              ['placeholder', 'Mensagem 3 (opcional)'],
              ['maxLength', 78],
            ],
          },
        },
        {
          input_type: 'input_text',
          label: 'Mensagem 4',
          field_name: 'message4',
          options: {
            class: 'mb-4',
            props: [
              ['placeholder', 'Mensagem 4 (opcional)'],
              ['maxLength', 78],
            ],
          },
        },
        {
          input_type: 'input_text',
          label: 'Mensagem 5',
          field_name: 'message5',
          options: {
            class: 'mb-4',
            props: [
              ['placeholder', 'Mensagem 5 (opcional)'],
              ['maxLength', 78],
            ],
          },
        },
      ],
    },
    {
      input_type: 'container',
      label: 'Descontos',
      options: { class: 'row mb-4' },
      items: [
        {
          input_type: 'input_text',
          label: 'Código Desconto 1',
          field_name: 'discount1_code',
          options: {
            props: [
              ['maxLength', 1],
              ['placeholder', 'Código'],
            ],
            class: 'col-md-3',
          },
        },
        {
          input_type: 'input_text',
          label: 'Data Limite Desconto 1',
          field_name: 'discount1_date',
          options: {
            props: [['type', 'date']],
            class: 'col-md-3',
          },
        },
        {
          input_type: 'input_text',
          label: 'Valor Desconto 1',
          field_name: 'discount1_value',
          options: {
            masks: ['currency'],
            props: [['type', 'number']],
            class: 'col-md-6',
          },
        },
      ],
    },
    {
      input_type: 'container',
      label: '',
      options: { class: 'row mb-4' },
      items: [
        {
          input_type: 'input_text',
          label: 'Código Desconto 2',
          field_name: 'discount2_code',
          options: {
            props: [
              ['maxLength', 1],
              ['placeholder', 'Código'],
            ],
            class: 'col-md-3',
          },
        },
        {
          input_type: 'input_text',
          label: 'Data Limite Desconto 2',
          field_name: 'discount2_date',
          options: {
            props: [['type', 'date']],
            class: 'col-md-3',
          },
        },
        {
          input_type: 'input_text',
          label: 'Valor Desconto 2',
          field_name: 'discount2_value',
          options: {
            masks: ['currency'],
            props: [['type', 'number']],
            class: 'col-md-6',
          },
        },
      ],
    },
    {
      input_type: 'container',
      label: '',
      options: { class: 'row mb-4' },
      items: [
        {
          input_type: 'input_text',
          label: 'Código Desconto 3',
          field_name: 'discount3_code',
          options: {
            props: [
              ['maxLength', 1],
              ['placeholder', 'Código'],
            ],
            class: 'col-md-3',
          },
        },
        {
          input_type: 'input_text',
          label: 'Data Limite Desconto 3',
          field_name: 'discount3_date',
          options: {
            props: [['type', 'date']],
            class: 'col-md-3',
          },
        },
        {
          input_type: 'input_text',
          label: 'Valor Desconto 3',
          field_name: 'discount3_value',
          options: {
            masks: ['currency'],
            props: [['type', 'number']],
            class: 'col-md-6',
          },
        },
      ],
    },
    {
      input_type: 'container',
      label: 'Multa',
      options: { class: 'row mb-4' },
      items: [
        {
          input_type: 'input_text',
          label: 'Código de Multa',
          field_name: 'fine_code',
          options: {
            props: [
              ['type', 'number'],
              ['placeholder', 'Código'],
            ],
            class: 'col-md-4',
          },
        },
        {
          input_type: 'input_text',
          label: 'Data de Multa',
          field_name: 'fine_date',
          options: {
            props: [['type', 'date']],
            class: 'col-md-4',
          },
        },
        {
          input_type: 'input_text',
          label: 'Valor de Multa',
          field_name: 'fine_value',
          options: {
            masks: ['currency'],
            props: [['type', 'number']],
            class: 'col-md-4',
          },
        },
      ],
    },
    {
      input_type: 'container',
      label: 'Mora',
      options: { class: 'row mb-4' },
      items: [
        {
          input_type: 'input_text',
          label: 'Código de Mora',
          field_name: 'fee_code',
          options: {
            props: [
              ['type', 'number'],
              ['placeholder', 'Código'],
            ],
            class: 'col-md-4',
          },
        },
        {
          input_type: 'input_text',
          label: 'Data de Mora',
          field_name: 'fee_date',
          options: {
            props: [['type', 'date']],
            class: 'col-md-4',
          },
        },
        {
          input_type: 'input_text',
          label: 'Valor de Mora',
          field_name: 'fee_value',
          options: {
            masks: ['currency'],
            props: [['type', 'number']],
            class: 'col-md-4',
          },
        },
      ],
    },
  ],

  recurring_boleto: [
    {
      input_type: 'container',
      label: 'Configuração da Recorrência',
      options: { class: 'row mb-4' },
      items: [
        {
          ...fieldTypes.boletoType,
          options: { ...fieldTypes.boletoType.options, class: 'col-md-4' },
        },
        {
          ...fieldTypes.currency,
          label: 'Valor do Boleto',
          field_name: 'amount',
          options: { ...fieldTypes.currency.options, class: 'col-md-4', props: [['required', true]] },
        },
        {
          input_type: 'input_select',
          label: 'Frequência',
          field_name: 'frequency',
          items: [
            { key: '', label: 'Selecione a frequência', disabled: true },
            { key: 'monthly', label: 'Mensal' },
            { key: 'bimonthly', label: 'Bimestral' },
            { key: 'quarterly', label: 'Trimestral' },
            { key: 'semiannual', label: 'Semestral' },
            { key: 'annual', label: 'Anual' },
          ],
          options: {
            props: [['required', true]],
            class: 'col-md-4',
          },
        },
      ],
    },
    {
      input_type: 'container',
      label: '',
      options: { class: 'row mb-4' },
      items: [
        {
          input_type: 'input_text',
          label: 'Data do Primeiro Vencimento',
          field_name: 'first_due_date',
          options: {
            props: [
              ['required', true],
              ['type', 'date'],
              ['min', new Date().toISOString().split('T')[0]],
            ],
            class: 'col-md-4',
          },
        },
        {
          input_type: 'input_text',
          label: 'Quantidade de Parcelas',
          field_name: 'installments',
          options: {
            props: [
              ['required', true],
              ['type', 'number'],
              ['min', 1],
              ['max', 60],
            ],
            class: 'col-md-4',
          },
        },
        {
          ...fieldTypes.myNumber,
          options: { ...fieldTypes.myNumber.options, class: 'col-md-4' },
        },
      ],
    },
    {
      input_type: 'container',
      label: 'Dados do Pagador',
      options: { class: 'row mb-4' },
      items: [
        {
          ...fieldTypes.name,
          label: 'Nome do Pagador',
          field_name: 'payer_name',
          options: { ...fieldTypes.name.options, class: 'col-md-6' },
        },
        {
          ...fieldTypes.cpfCnpj,
          field_name: 'payer_document',
          options: { ...fieldTypes.cpfCnpj.options, class: 'col-md-6' },
        },
      ],
    },
    {
      input_type: 'container',
      label: '',
      options: { class: 'row mb-4' },
      items: [
        {
          ...fieldTypes.phone,
          field_name: 'payer_phone',
          options: { ...fieldTypes.phone.options, class: 'col-md-6' },
        },
        {
          ...fieldTypes.email,
          label: 'E-mail',
          field_name: 'payer_email',
          options: {
            ...fieldTypes.email.options,
            class: 'col-md-6',
            props: [
              ['required', true],
              ['type', 'email'],
            ],
          },
        },
      ],
    },
  ],
};

export const getReceiptByCategory = (category) => {
  return receipts[category] || {};
};

export const createDynamicField = (baseField, overrides = {}) => {
  return {
    ...baseField,
    ...overrides,
    field_name: overrides.field_name || `${baseField.field_name}_${generateId()}`,
  };
};
