...
export const isFieldRequired = (field) => {
  return field?.options?.props?.some(prop =>
    Array.isArray(prop) && prop[0] === 'required' && prop[1] === true
  );
};
...
export const applyMask = (value, mask) => {
  ...
  const selectedMask = masks[mask] || mask;
  let maskedValue = '';
  let valueIndex = 0;

  for (let i = 0; i < selectedMask.length && valueIndex < value.length; i++) {
    if (selectedMask[i] === '0') {
